...
# k8s-worker1 cert details
WORKER_1_CERT=/var/lib/kubelet/k8s-worker1.crt
WORKER_1_KEY=/var/lib/kubelet/k8s-worker1.key

# k8s-worker1 kubeconfig location
WORKER_1_KUBECONFIG=/var/lib/kubelet/kubeconfig

# k8s-worker1 kubelet config location
WORKER_1_KUBELET=/var/lib/kubelet/kubelet-config.yaml

# Systemd k8s-worker1 kubelet location
SYSTEMD_WORKER_1_KUBELET=/etc/systemd/system/kubelet.service

# kube-proxy k8s-worker1 location
...
check_cert_only()
{
    ...
    # k8s-worker2 auto cert is a .pem
...
echo "This script will validate the certificates in master as well as k8s-worker1 nodes. Before proceeding, make sure you ssh into the respective node [ Master or k8s-worker1 ] for certificate validation"
echo
...
echo "  4. Verify kubeconfigs and PKI on k8s-worker1 Node after step 10"
echo "  5. Verify kubeconfigs and PKI on k8s-worker2 Node after step 11"
echo
...
case $value in
  ...
  1)
    if ! [ "${HOST}" = "k8s-master1" -o "${HOST}" = "k8s-master2" ]
    then
        printf "${FAILED}Must run on k8s-master1 or k8s-master2${NC}\n"
        exit 1
    fi

    echo -e "The selected option is $value, proceeding the certificate verification of Master node"

    CERT_LOCATION=$HOME/certs
  ...
  4)
    if ! [ "${HOST}" = "k8s-worker1" ]
    then
        printf "${FAILED}Must run on k8s-worker1${NC}\n"
        exit 1
    fi
    ...
    check_cert_only "ca" $SUBJ_CA $CERT_ISSUER
    check_cert_and_key "kube-proxy" $SUBJ_KP $CERT_ISSUER
    check_cert_and_key "k8s-worker1" "Subject:CN=system:node:k8s-worker1,O=system:nodes" $CERT_ISSUER
  ...
  5)
    if ! [ "${HOST}" = "k8s-worker2" ]
    then
        printf "${FAILED}Must run on k8s-worker2${NC}\n"
        exit 1
    fi
    ...
    check_cert_only "kubelet-client-current" "Subject:O=system:nodes,CN=system:node:k8s-worker2" $CERT_ISSUER
    check_kubeconfig "kube-proxy" "/var/lib/kube-proxy" "https://${LOADBALANCER}:6443"
    ;;
